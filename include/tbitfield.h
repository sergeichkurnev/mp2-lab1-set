...
//TELEM bit_width(int kol)
//{
//	return floor(log(kol) / log(2))+1;
//}
...
private:
	...
	const TELEM BitsInElem = sizeof(TELEM);//numeric_limits<TELEM>::digits;//сколько бит в TELEM
	const TELEM shiftSize = floor(log(BitsInElem - 1) / log(2)) + 1;//bit_width(BitsInElem - 1);//насколько нужно сместиться для побитовой операции
	// методы реализации
